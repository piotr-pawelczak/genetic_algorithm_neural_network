...
class GeneticAlgorithm():
    ...
    def __init__(self, population_size, x_train, y_train):
    ...
    def get_fitness(self, network: NeuralNetwork, chromosome: np.ndarray, metric_type='accuracy', batch_size=10) -> float:
        """
        ...
        if metric_type == 'accuracy':
            fitness = network.get_accuracy(self.x_train, self.y_train, batch_size)
        elif metric_type == 'loss':
            loss = network.get_loss(self.x_train, self.y_train, batch_size)
